// Package rpc 承载 api/stateflux/task/v1 的 gRPC adapter（§6.3、§7），把两种 wire 形态
// 适配为 eventbus/channel 的两种能力；本包只做协议编解码，不解释任务语义：
//
//   - unary Execute：半双工 request/reply（channel.Requester）。调度侧发送 TaskMessage 并
//     阻塞等待一个结果，响应适配为 ResultEvent 后送入同一个 result topic（§5.5）；
//   - 双向 ResultStream：全双工（channel.Publisher + channel.Subscriber）。worker 发布
