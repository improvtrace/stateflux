...
// 白名单）；task_id / idempotency_key 一律禁止入标签。通信面指标（eventbus.*）只描述通道
// 行为，不作为任务正确性来源——正确性信号来自 PG（对账重置与死信），因为通道本身被设计为
// 可丢失、可重复（§1.2.1、§9.3）。
//
// 水位型指标（wal.backlog）采用同步 Int64Gauge——采集点位收敛在既有写路径，在测量点直接
// 记录，语义与 ObservableGauge 相同且免去回调装配（§6.4）。
package obs
...
type Metrics struct {
	...
	// stateflux.reconcile.resets —— 对账：R1/R3 重置与死信次数（reason 标签，核心健康信号）。
