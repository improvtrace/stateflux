...
func (TaskProcessing) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("claimed_node").Comment("claiming node (current claim, diagnostic); executors hold no authoritative state (§1.2.4)").Default(""),
		...
		field.String("label").Comment("scheduling target-node attribute: node matching label, filtered at claim; retained in-flight (§3.1/§5.2)").Default(""),
		field.Int16("hash_bucket").Comment("scheduling target-node attribute: hash bucket (0-255), 0 = unrestricted; semantics defined by the integrator, framework does exact matching only, filtered at claim; retained in-flight (§3.1/§5.2)").Default(0),
