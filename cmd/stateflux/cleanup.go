...
func boundedCleanup(name string, timeout time.Duration, fn func()) func() {
	...
	return func() {
		log.Printf("stateflux: [cleanup] %s: started", name)
		start := time.Now()
		...
		select {
		case <-done:
			log.Printf("stateflux: [cleanup] %s: done in %s", name, time.Since(start).Round(time.Millisecond))
		case <-time.After(timeout):
			log.Printf("stateflux: [cleanup] %s: exceeded %s; abandoning and continuing shutdown", name, timeout)
		}
	}
}
